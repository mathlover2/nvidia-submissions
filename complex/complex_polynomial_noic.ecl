:- module(complex_polynomial_noic).
:- use_module(complex_noic).

:- export complex_polynomial/3.
:- export durand_kerner/4.

complex_polynomial(L, X, R) :-
        complex_polynomial0(L, X, c(0,0), R), !.

complex_polynomial0([], _, Res, Res).
complex_polynomial0([H], _, Res1, Res) :-
        complex_mul(Res1, X, Tmp),
        complex_add(Tmp, H, Res).
complex_polynomial0([H1, H2], X, Res1, Res) :-
        complex_mul(Res1, X, Tmp1),
        complex_add(Tmp1, H1, Tmp2),
        complex_mul(Tmp2, X, Tmp3),
        complex_add(Tmp3, H2, Res).
complex_polynomial0([H | T], X, Res1, Res) :-
        complex_mul(Res1, X, Tmp),
        complex_add(Tmp, H, Res2),
        complex_polynomial0(T, X, Res2, Res).
